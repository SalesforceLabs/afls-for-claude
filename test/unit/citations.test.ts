...
describe("citation-map.ts", () => {
  ...
  // ==========================================================================
  describe("citationMap (Guides, Exercises, Troubleshooting)", () => {
    it("has no Official Help entries (now served live via MCP)", () => {
      const helpEntries = Object.values(citationMap).filter(
        (e) => e.category === "Official Help",
      );
      expect(helpEntries.length).toBe(0);
    });

    it("has 3 Guide entries", () => {
  ...
  describe("getCitation()", () => {
    it("returns undefined for a bare module slug (Official Help now served live via MCP)", () => {
...
describe("formatCitation()", () => {
  ...
  it("uses Documentation fallback (no Link) for a bare module slug", () => {
  ...
  it("uses Documentation source (not Official Help) when no moduleName for a bare module slug", () => {
    const result = formatCitation("account-management");
    expect(result).toContain("Documentation");
    expect(result).not.toContain("Official Help");
  });
