...
export function register(server: McpServer) {
    ...
    async ({ targetOrg }) => {
      ...
      try {
        ...
        if (!catResult.success || !catResult.data?.records?.length) {
          ...
          const available =
            allCatResult.success && allCatResult.data?.records?.length
              ? (allCatResult.data.records as Array<Record<string, unknown>>)
                  .map((c) => c.Category)
                  .join(", ")
              : "none found";
          return {
  ...
  server.tool(
    ...
    "Add a new navigation tab to AFLS Mobile via Admin Console > Mobile > UI Settings. Creates a LifeSciConfigRecord of type Tab with field values and optional profile assignments.",
    {
      label: z
        .string()
        .describe("Display name for the component (MasterLabel), e.g. 'Accounts Tab'"),
      developerName: z
        .string()
        .describe(
      ...
      tabOrder: z
        .number()
        .int()
        .describe("Position number — lower values appear further left (e.g. 1, 2, 3)"),
      profiles: z
        .array(z.string())
        .optional()
        .describe("Optional: profile names to assign (e.g. ['Field Sales Representative'])"),
      isActive: z
        .boolean()
        .optional()
        .default(true)
        .describe("Whether the tab is active on creation (default: true)"),
