...
import { z } from "zod";
import { runSoqlQuery, createRecord, deleteRecord } from "../salesforce/cli.js";
...
export function register(server: McpServer) {
  server.tool(
    ...
    {
      filterBy: z
        .enum(["all", "profile", "permission-set", "territory", "no-territory", "no-psl"])
        .optional()
        .default("all")
        .describe(
      ...
      filterValue: z
        .string()
        .optional()
        .describe("Partial match value for profile, permission-set, or territory filters"),
      targetOrg: z
        .string()
        .optional()
  ...
  server.tool(
    ...
    "Assign a permission set to one or more users. Looks up the permission set by label or API name, resolves usernames to IDs if needed, skips users who already have the assignment, and creates PermissionSetAssignment records.",
    {
      ...
      userIds: z
        .array(z.string())
        .optional()
        .describe("Array of User IDs to assign the permission set to"),
      usernames: z
        .array(z.string())
        .optional()
        .describe(
      ...
      targetOrg: z
        .string()
        .optional()
        .describe("Optional: specific org. Uses current target org if not specified."),
    },
      ...
      try {
        ...
        if (usernames?.length) {
          ...
          if (userResult.success && userResult.data?.records) {
            ...
            const foundUsernames = userResult.data.records.map((r) =>
              String((r as Record<string, unknown>).Username)
            );
  ...
  server.tool(
    ...
    "Remove a permission set assignment from one or more users. Finds existing PermissionSetAssignment records and deletes them.",
    {
      ...
      userIds: z
        .array(z.string())
        .optional()
        .describe("Array of User IDs to remove the permission set from"),
      usernames: z
        .array(z.string())
        .optional()
        .describe(
